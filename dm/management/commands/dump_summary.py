...
class Command(BaseCommand):
    ...
    def handle(self, *args, **option):
        ...
        # id = 1900017854
        # np = NaturalPerson.objects.get(person_id=User.objects.get(username=id))
        # data = cal_act(np)
        # print(data)
        ...
        with open('test_data/summary_overall_2023.json', 'w', encoding='utf-8') as f:
            json.dump(overall_info, f, default=datetime_converter, ensure_ascii=False)

        # 导出个人信息
        summary_info = person_infos()
        with open('test_data/summary2023.json', 'w', encoding='utf-8') as f:
            json.dump(summary_info, f, default=datetime_converter, ensure_ascii=False)
